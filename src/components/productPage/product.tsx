...
// Tính số lượng sản phẩm cho từng danh mục
  const categoryProductCount = categories.reduce((acc:{ [key: string]: number }, category:ICategory) => {
    acc[category.name] = products.filter((product:IProduct) => product.category === category.name).length;
    return acc;
...
// Sắp xếp danh mục theo số lượng sản phẩm giảm dần
  const sortedCategories = categoriesWithCount.sort((a: ICategory & { productCount: number }, b: ICategory & { productCount: number }) => b.productCount - a.productCount);
  ...
  return (
    <div>
      <div className='w-full h-[163px] flex items-center [background:linear-gradient(97.31deg,_#b5dcb0,_rgba(249,_243,_238,_0))]'>
        <h1 className='mx-auto w-full px-12 py-6 text-[40px] text-[#505F4E] font-Baloo font-medium'>Töpfe & Behälter</h1>
      </div>
      {/* End banner */}
      <div className="flex justify-around items-center py-12 bg-white ">
        {displayedCategories.map((category:ICategory)=>(
          <div key={category.id} className="flex space-x-4 items-center p-4 bg-[#C6E3C1] rounded-md shadow-md">
          <img src={category.image} alt="Eckige Töpfe" className="w-20 h-auto" />
